...
(function() {
    AjaxUtil.ajaxSetup = function() {
        jQuery.ajaxSetup({
            statusCode: {
                // SC_GONE
                410: function(xhr) {
                    // var refreshPage = xhr.getResponseHeader("refreshPage");
                    ...
                    //         if (message && message !== "") {
                    //             window.confirm(decodeURIComponent(xhr.getResponseHeader("message")));
                    //         }
                    //         window.self.location = '/relogin.html';
                    //         return;
                    //     }
                    ...
                    // var redirectPath = xhr.getResponseHeader("redirectPath");
            ...
            beforeSend: function(xhr, setting) {
                if (typeof (setting.dataType) == 'undefined' || !setting.dataType) {
                    //Default : JSON
    ...
    ;
    /*** postAjax ***/
    AjaxUtil.doAjax = function(options, that) {
        if ('' == options.type) {
            options.type = "POST";
        }
        options.dataType = "json";

        // -- JSESSIONID Re-write start
        var srw = "";

        try {
            if (typeof PageConfig !== 'undefined' && typeof PageConfig.srw !== 'undefined') {
                srw = PageConfig.srw;
            } else if (typeof parent.PageConfig !== 'undefined' && typeof parent.PageConfig.srw !== 'undefined') {
                srw = parent.PageConfig.srw;
            }
        } catch (e) {}

        if (srw != "") {
            options.url = options.url + ";jsessionid=" + srw;
        }
        // -- JSESSIONID Re-write end
        ...
        if (options.error == undefined) {
            options.error = function(response) {
                try {
                    ...
                    //		showWarning('post Ajax error!');
                    //	}
                    if (typeof response.responseText != 'undefined' && response.responseText.indexOf(' html ') > -1) {
                        // possible session timeout reload page
                        location.reload(true);
                    }
                } finally {
                    if (that != null && typeof (that.check) != 'undefined') {
                        that.check();
